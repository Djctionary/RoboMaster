...
void saveIntegersToFile(int num1, int num2, int count,int number) {
    ...
    file  <<"The"<<number<<"photo, " count << "times: (" << num1 << ", " << num2 << ")" << std::endl;
...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch,int number) {
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      count++;
      saveIntegersToFile(x, y, count, number);
      cvCircle(img,cvPoint(x,y),5,cvScalar(0, 201, 87), -1, 4, 0);//这个是在输出的中心点处画圆
      cv::line(img,Point(r.x,r.y),Point(r.x+r.width,r.y+r.height),Scalar(0, 0, 255),3,8,0);
      cv::line(img,Point(r.x+r.width,r.y),Point(r.x,r.y+r.height),Scalar(0, 0, 255),3,8,0);
