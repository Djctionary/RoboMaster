...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
    ...
    // Draw bounding boxes
    static long long int number=1;
    draw_bbox(img_batch, res_batch);//生成中心点坐标存入本地文本中
    number++;
    
    // Save images
    for (size_t j = 0; j < img_batch.size(); j++) {
      string save_path = "../output_images";
      cv::imwrite(save_path + "/" + img_name_batch[j], img_batch[j]);
    }
  }
