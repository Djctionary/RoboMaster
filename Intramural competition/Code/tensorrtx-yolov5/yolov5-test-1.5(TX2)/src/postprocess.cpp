...
#include "utils.h"
#define PI 3.1415
void saveIntegersToFile(int num1, int num2, int count,int number,double angle, double pid_result) {
    ...
    file  <<"The "<<number<<" photo, "<< count << "times: (" << num1 << ", " << num2 << ")" <<  "  angle = "<< angle<<"°"<<" pid_result: "<<pid_result<<std::endl;
...
double PID(double x, double y)
{
    ...
    int Kp_1000 = 1001, Ki_1000 = 250, Kd_1000 = 6;
        // PID误差为theta偏转角
        pid_error = atan2((center - x), (640 - y));
        // cout << "error:" << pid_error << endl;
        // if(abs(pid_error-0)<0.002)
        // {
        //     cout<<(center - x)<<"_________________"<<(320 - y)<<endl;
        // }
        // 积分
...
double draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, int number) {
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      double angle=atan((centerX-x)/y)*180/PI;
      std::string class_id ="arrow";
      double pid_result = PID(x,y);
      saveIntegersToFile(x, y, count, number, angle, pid_result);
      cv::circle(img,cv::Point(x,y),10,cv::Scalar(0, 201, 87), -1,8,0);//ÕâžöÊÇÔÚÊä³öµÄÖÐÐÄµãŽŠ»­Ô²
