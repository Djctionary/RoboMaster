...
#include "utils.h"
#define PI 3.1415
void saveIntegersToFile(int num1, int num2, int count,int number,double angle) {
    ...
    file  <<"The "<<number<<" photo, "<< count << "times: (" << num1 << ", " << num2 << ")" <<  "  angle = "<< angle<<"°"<<std::endl;
...
double draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, int number) {
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      double angle=atan((centerX-x)/f)*180/PI;
      std::string class_id ="arrow";
      saveIntegersToFile(x, y, count, number,angle);
      cv::circle(img,cv::Point(x,y),10,cv::Scalar(0, 201, 87), -1,8,0);//Õâ¸öÊÇÔÚÊä³öµÄÖÐÐÄµã´¦»­Ô²
